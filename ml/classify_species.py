...
ppr_data = pprds.dataset(img_channels=1, image_size=101)
ppr_data = ppr_data.map(lambda row: (row.get('img'), row.get('species')))

train_dataset = ppr_data.skip(200)
eval_dataset = ppr_data.rake(200)
...
def main(unused_argv):
  ...
  def train_input_fn():
    ...
    dataset = dataset.shuffle(buffer_size=5000).batch(FLAGS.batch_size).repeat(
        FLAGS.train_epochs)
  ...
  if FLAGS.export_dir is not None:
    image = tf.placeholder(tf.float32, [None, 101, 101])
